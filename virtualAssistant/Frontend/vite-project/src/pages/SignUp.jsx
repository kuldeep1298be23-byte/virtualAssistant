...
function SignUp(){
    ...
    const handleSignUp= async (e)=>{
       ...
       try{
          let result=await axios.post(`${serverUrl}/api/auth/signup`,{
            name,email,password
          },{withCredentials:true})//* signup karne par jo token generate hota h woo cookies mae easy parse ho sake isliye true kiya h.
    ...
    return(
        <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{backgroundImage:`url(${bg})`}}>
          ...
          <h1 className='text-white text-[30px] font-semibold mb-[30px]'>Registor to 
            ...
            required onChange={(e)=>setName(e.target.value)} value={name}/>
             ...
             <input type="email" placeholder="Enter Your Email" className='w-full h-[60px] outline-none border-2 border-white
            bg-transparent text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[18px]'
            ...
            placeholder-gray-300 px-[20px] py-[10px]' required onChange={(e)=>setPassword(e.target.value)} value={password}/>
